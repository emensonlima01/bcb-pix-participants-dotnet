...
public sealed class BcbCsvDataSourceReader(
    HttpClient httpClient, 
    IOptions<PixParticipantsOptions> options) : IDataSourceReader
{
    private static readonly Encoding CsvEncoding = Encoding.GetEncoding("Windows-1252");
    private readonly PixParticipantsOptions _options = options.Value;

    public async IAsyncEnumerable<string> ReadLinesAsync(
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(_options.CsvUrl))
        {
            throw new InvalidOperationException("Pix participants CSV URL is not configured.");
        }

        var today = DateTime.Now;
        var response = await GetResponseAsync(_options.CsvUrl, today, cancellationToken);

        if (response.StatusCode == HttpStatusCode.NotFound)
        {
            response.Dispose();
            response = await GetResponseAsync(_options.CsvUrl, today.AddDays(-1), cancellationToken);
        }
        ...
        using (response)
        {
            ...
            await foreach (var line in ReadLinesFromPipeAsync(reader, cancellationToken))
            {
                yield return line;
            }
        }
    ...
    private static async IAsyncEnumerable<string> ReadLinesFromPipeAsync(
        PipeReader reader,
